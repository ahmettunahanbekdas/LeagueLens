...
class Services {
    ...
    func downloadLeagues(completion: @escaping ([ResponseLeague]?) -> ()) {
        ...
        guard let url = URL(string: Endpoint.leagues()) else {
    ...
    func downloadLeaguesDetails(id: Int, completion: @escaping ([LeagueStanding]?) -> ()) {
        let headers = Endpoint.APIKey()
        guard let url = URL(string: Endpoint.leagueDetailTeams(id: id)) else {
    ...
    func searchLeagues(with query: String,completion: @escaping ([ResponseLeague]?) -> ()) {
        let headers = Endpoint.APIKey()
        guard let url = URL(string: Endpoint.searchLeague(with: query)) else {
            print("Football API URL Error")
            completion(nil)
            return
        }
